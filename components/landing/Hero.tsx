...
const quickTags = [
  { label: "🛡️ Nominus (Siap Ganti)", href: "/listings?q=Nominus" },
  { label: "⭐ OVR 90+ Full Legend", href: "/listings?q=90" },
  { label: "🔥 Messi Big Time / Ronaldo", href: "/listings?q=Messi" },
  { label: "👑 MLBB Mythic Immortal", href: "/listings?q=Mythic" },
  { label: "💰 Budget < 350rb", href: "/listings?q=350" },
];
...
export function Hero() {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-blue-50/70 via-white to-slate-50 border-b border-slate-200 pt-10 pb-16">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 pt-6 pb-4 z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-white px-3.5 py-1 text-xs text-blue-700 mb-5 shadow-sm">
            ...
            <span className="font-semibold">Platform Rekber & Marketplace Game Terpercaya</span>
          </div>

          {/* Glints-style Heading */}
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black tracking-tight text-slate-900 mb-4 leading-[1.15]">
            Temukan Akun Impianmu, <br className="hidden sm:inline" />
          ...
          <p className="text-slate-600 max-w-2xl mx-auto text-base sm:text-lg mb-8 leading-relaxed">
            Marketplace akun eFootball & game kompetitif terbesar. Bebas ripper, dana tersimpan di admin escrow terverifikasi, akun langsung berpindah tangan dengan tenang.
          </p>

          {/* Glints-Style Multi-field Search Widget */}
          <div className="bg-white rounded-2xl border border-slate-200 p-2 sm:p-2.5 shadow-xl shadow-blue-950/5 mb-6 text-left">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                let url = `/listings?q=${encodeURIComponent(searchQuery)}`;
                if (selectedGame !== "all") {
            ...
            >
              ...
              <div className="flex-1 flex items-center gap-2.5 px-3.5 py-2.5 bg-slate-50 md:bg-white rounded-xl border md:border-transparent border-slate-200 focus-within:border-blue-500 focus-within:bg-white transition-all">
                ...
                <div className="w-full">
                  <label className="hidden text-[10px] uppercase font-bold text-slate-400 tracking-wider">Cari Akun</label>
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="Ketik squad, pemain (Messi/Neymar), OVR, nama admin..."
              ...
              <div className="md:w-56 flex items-center gap-2 px-3.5 py-2.5 bg-slate-50 md:bg-white rounded-xl border md:border-transparent md:border-l md:border-slate-200 border-slate-200">
        ...
        {/* 4 Stats Cards — Clean White Glints Style */}
